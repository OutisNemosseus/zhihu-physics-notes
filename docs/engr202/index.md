...
[![Problem 3 补充题目：C 求 Iy，D 求 Ix，并根据各支路的参考方向组合网孔电流](../assets/images/engr202/problem-3-questions-c-d.png)](../assets/images/engr202/problem-3-questions-c-d.png)
...
- **B：** 求三个网孔电流的数值。
- **C：** 求沿 $R_1$ 向下的支路电流 $I_y$。根据支路位置与参考方向，由网孔电流得到实际支路电流。
- **D：** 求沿 $R_5$ 向上的支路电流 $I_x$。
...
$R_1$ 只属于右上网孔，且 $I_y$ 的向下参考方向与 $i_2$ 在该支路上的方向相同，因此：

$$\boxed{I_y=i_2=-\frac{77}{93}\ \mathrm A\approx-0.827957\ \mathrm A.}$$

负号表示实际电流沿 $R_1$ **向上**，大小为 $0.827957\ \mathrm A$。题目提醒按支路组合网孔电流；只有一个网孔经过的支路，其支路电流可以直接等于该网孔电流。

### D · 求 $I_x$

$R_5$ 是左侧与右下两个网孔的公共电阻。在此支路，$i_3$ 向上、$i_1$ 向下。以题图 $I_x$ 的向上方向为正：

$$\boxed{I_x=i_3-i_1=-\frac{52}{93}-\frac{50}{93}=-\frac{34}{31}\ \mathrm A\approx-1.096774\ \mathrm A.}$$

负号表示实际电流沿 $R_5$ **向下**，大小为 $1.096774\ \mathrm A$。
...
| C · $I_y=i_2$ | `Iy_A` | **−0.827957 A** |
| D · $I_x=i_3-i_1$ | `Ix_A` | **−1.096774 A** |

打开 `Problem3_Log.asc`，点击 **Run** 后按 **Ctrl+L**，`iy_a` 对应 C 问，`ix_a` 对应 D 问。这两个测量已包含在文件中。
...
整包包含三道原题截图、第三题 C/D 补充截图、每题的 `.asc` 和 `.cir` 文件、计算推导及验证记录。

...
教材答案来自本题解答截图。教材直流部分忽略基极电流，小信号部分仍使用 $\beta=120$；SPICE 则保留直流基极电流。教材还取 $V_T=26\,\mathrm{mV}$ 并作数值取整。

| 量 | 教材答案 | 原始 LTspice 已运行结果 | 保留基极电流的方程参考值 |
|---|---:|---:|---:|
| $I_{D1}$ | 0.405 mA | 0.41115985 mA | 0.41116 mA |
| $I_{C2}$（大小） | 0.828 mA | 0.81921357 mA | 0.81921 mA |
| $V_{OQ}$ | 约 1.4803 V，由教材 $V_{GS}$ 推得 | 1.47644818 V | 1.47645 V |
| $A_v$ | 0.8812 | 0.88214874 | 0.88197 |
| (c) 输出电阻，包含 $R_L$ | 143 Ω | 137.09068 Ω，受原测量精度影响 | 141.63622 Ω |
| 补充：不含 $R_L$ 的输出电阻 | 未给出 | 154.77221 Ω，受原测量精度影响 | 160.59078 Ω |

直流电流差异约 1%–1.5%，符合两种偏置近似的区别；增益十分接近。原始日志的输出电阻不能作为修正版已验证的数值。
...
以下记录来自修正前的真实运行；省略本机文件路径。修正版的重跑结果待补充。

```text
id1_ma: (1000*Id(M1))=0.411159853684 at 0.0005
...
rof_ohm: 1/(1/rout_loaded_ohm-1/1200)=154.772205819
```
